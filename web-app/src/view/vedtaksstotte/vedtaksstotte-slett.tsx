...
export const VedtaksstotteSlett = () => {
	...
	const handleSlett14aVedtak = () => {
		...
		if (window.confirm('Sikker på at du ønsker å slette § 14 a-vedtak for person ' + fnr + '?')) {
			slett14avedtak(journalpostId, fnr, ansvarligVeileder, slettVedtakBestillingId)
				.then(() => {
					successToast(
						`Sletting av § 14 a-vedtak for person ${fnr} med journalpostId ${journalpostId} er utført.`
					);
					setJournalpostId('');
					setFnr('');
				...
				.catch(() =>
					errorToast(
						`Klarte ikke å slette § 14 a-vedtak for person ${fnr} med journalpostId ${journalpostId}`
					)
				);
		}
	};
	...
	return (
		<Card title="Slett § 14-vedtak" className="small-card" innholdClassName="card__content">
			<BodyShort className="blokk-xxs" spacing={true}>
				Det å slette § 14-a vedtak skal kun gjøres dersom vedtak er fattet på feil person. Om dette ikke er
				tilfelle skal man bare fatte et nytt vedtak.
			</BodyShort>
			<TextField
			...
			/>
			<TextField
				label="Fødselsnummer § 14 a-vedtak som skal slettes er fattet på"
				value={fnr}
				onChange={e => setFnr(e.target.value)}
			/>
			<TextField
				label="Ident på veileder som har bestilt sletting av § 14 a-vedtak"
				value={ansvarligVeileder}
				onChange={e => setAnsvarligVeileder(e.target.value)}
			/>
			<TextField
				label="Jira/FAGSYSTEM-nummer hvor bestilling av sletting av § 14 a-vedtak finnes."
				description="Eksempel: FAGSYSTEM-1234556"
				value={slettVedtakBestillingId}
				onChange={e => setSlettVedtakBestillingId(e.target.value)}
			/>

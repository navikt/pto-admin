...
export const BrukerStatusCard = () => {
	const [brukerStatus, setBrukerStatus] = useState<BrukerStatusDto | null>(null);
	...
	const fetchBrukerStatus = async (e: React.SubmitEvent<HTMLFormElement>) => {
		...
		try {
			...
			setBrukerStatus(response.data.brukerStatus);
		} catch (e: any) {
			setError(e?.toString());
		} finally {
			setIsLoading(false);
					...
					<div className="bg-gray-100 p-3 border border-gray-200 rounded">
						...
						{brukerStatus.manuell ? (
							<>
								...
								{brukerStatus.manuell.begrunnelse && (
									<div>Begrunnelse: {brukerStatus.manuell.begrunnelse}</div>
								)}

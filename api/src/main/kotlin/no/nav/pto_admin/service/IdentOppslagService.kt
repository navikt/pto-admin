...
class IdentOppslagService(
    ...
    fun aktorIdTilFnr(aktorId: AktorId): Fnr {
        return pdlPipClient.hentBrukerInfo(aktorId.get())
            .identer.identer.first { !it.historisk && it.gruppe == IdentGruppe.FOLKEREGISTERIDENT }
    ...
    fun fnrTilAktorId(fnr: Fnr): AktorId {
        return pdlPipClient.hentBrukerInfo(fnr.get())
            .identer.identer.first { !it.historisk && it.gruppe == IdentGruppe.AKTORID }
            .let { AktorId.of(it.ident) } ?: throw RuntimeException("Fant ikke fnr for aktorId")
    }

    fun hentAlleIdenter(eksternBrukerId: EksternBrukerId): List<Ident> {
        return pdlPipClient.hentBrukerInfo(eksternBrukerId.get())
            .identer.identer
    }
